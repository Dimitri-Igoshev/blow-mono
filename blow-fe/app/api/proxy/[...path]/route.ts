...
const filterRequestHeaders = (source: Headers) => {
  ...
  source.forEach((value, key) => {
    ...
    if (!hopByHopHeaders.has(lowerKey) && !requestExcludedHeaders.has(lowerKey)) {
...
const filterResponseHeaders = (source: Headers) => {
  ...
  source.forEach((value, key) => {
    const lowerKey = key.toLowerCase();
    if (!hopByHopHeaders.has(lowerKey) && !responseExcludedHeaders.has(lowerKey)) {
      result.append(key, value);
    }
  });
  return result;
...
const appendSetCookieHeaders = (target: Headers, response: Response) => {
  // @ts-expect-error getSetCookie доступен в рантайме Next.js (Node)
  ...
  if (fallbackCookie) target.append("set-cookie", fallbackCookie);
};
...
const proxy = async (
  req: NextRequest,
  { params }: { params: { path: string[] } } // ← исправлено: path обязателен
) => {
  if (!API_BASE) {
  ...
  try {
    const upstreamResponse = await fetch(targetUrl, await createRequestInit(req));
