      ...
      <div className="flex w-full flex-col px-3 md:px-9 pt-[84px] gap-6 pb-[50px]">
        <div className="w-full">
          ...
          <p className="text-sm sm:text-base mt-4">
            <strong>18+</strong> Полезные статьи для пользователей нашей платформы.
          </p>

          <section className="mt-9 grid grid-cols-1 sm:grid-cols-2 gap-6">
            <NextLink href={"/soderzhanki-vygoda-dlya-muzhchin"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> В чём выгода для мужчин
              </h2>
            </NextLink>

            <NextLink href={"/devushki-v-sugar-dating-vygody"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Девушки и формат-содержанки
              </h2>
            </NextLink>

            <NextLink href={"/etiket-i-dolgosrochnye-otnosheniya-win-win"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Этикет долгосрочных отношений
              </h2>
            </NextLink>

            <NextLink href={"/kak-obsuzhdat-podderzhku-vzroslye-otnosheniya"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Как обсуждать поддержку
              </h2>
            </NextLink>

            <NextLink href={"/krasnye-flagi-i-moshenniki-v-sugar-dating"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Красные флаги и мошенники
              </h2>
            </NextLink>

            <NextLink href={"/pervaya-vstrecha-checklist"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Первая встреча
              </h2>
            </NextLink>

            <NextLink href={"/silnaya-anketa-soderzhanki"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Сильная анкета содержанки
              </h2>
            </NextLink>

            <NextLink href={"/profil-sponsora"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Профиль спонсора
              </h2>
            </NextLink>

            <NextLink href={"/perepiska-scenarii"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
                <TiInfoLarge className="text-primary" /> Переписка
              </h2>
            </NextLink>

            <NextLink href={"/pravila-bezopasnost-moskva-rf"}>
              <h2 className="text-xl sm:text-2xl font-semibold cursor-pointer hover:text-primary flex items-center gap-2">
