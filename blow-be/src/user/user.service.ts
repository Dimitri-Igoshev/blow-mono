...
export class UserService {
  ...
  async publicCount() {
    return this.userModel
      .countDocuments({
        status: UserStatus.ACTIVE, // подправь под свои статусы
        // не банен и т.д.
      })
      .exec()
      .then((count) => ({ count }));
  ...
  async listPublicForSitemap(skip = 0, limit = 45000) {
    const docs = await this.userModel
      .find(
        { status: UserStatus.ACTIVE }, // + свои условия
        {
          slug: 1,
          shortId: 1,
          firstName: 1,
  ...
  async getUsersCount() {
    ...
    const [all, active, fake, real, fakeOnline, realOnline] = await Promise.all(
      [
  ...
  async massMailing() {
    // 1) Найти активных пользователей, исключая фейков:
    //    {$ne: true} покроет false, null и отсутствие поля.
    ...
    for (const user of users) {
      ...
      try {
        await this.mailerService.sendMail({
          to: user.email,
          from: 'support@blow.ru',
          subject: 'Акция! Получите 500₽ на баланс BLOW.RU',
...
• Напишите в Telegram админке “Задание выполнено” + ваш email: https://t.me/blowadmin
• Дождитесь проверки
...
<head>
  <meta charset="UTF-8" />
  <title>Акция! Получите 500₽ на баланс BLOW.RU</title>
</head>
<body style="margin:0; padding:0; background-color:#f9f9f9;">
  <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="background-color:#f9f9f9;">
    <tr>
      <td align="center" style="padding: 40px 0;">
        <table role="presentation" cellpadding="0" cellspacing="0" width="600" style="background-color:#ffffff; border-radius:8px; overflow:hidden; font-family: Arial, 'Montserrat', sans-serif;">
          <!-- Header -->
          <tr>
            <td align="center" bgcolor="#e31e24" style="padding: 20px;">
              <img src="https://blow.igoshev.de/blow-logo.png" alt="BLOW Logo" width="160" style="display:block;" />
          ...
          <tr>
            <td style="padding: 30px 40px; color: #333333; font-size: 16px; line-height: 1.5;">
              <h2 style="margin: 0 0 20px 0; font-size: 22px; font-weight: 600;">Акция! Получите 500₽ на баланс BLOW.RU</h2>
              <p style="margin: 0 0 16px 0;">Здравствуйте, ${firstName}!</p>
              <p style="margin: 0 0 16px 0;">Чтобы получить бонус, выполните простые шаги:</p>
              ...
              <ul style="margin:0 0 16px 20px; padding:0;">
                ...
                <li style="margin-bottom:8px;">Запишите голосовое сообщение</li>
                <li style="margin-bottom:8px;">
                  Напишите Администрации: «Задание выполнено» + ваш E-mail в Telegram —
                  <a href="https://t.me/blowadmin" style="color:#e31e24; text-decoration:none;">@blowadmin</a>
                </li>
                <li style="margin-bottom:8px;">Дождитесь проверки</li>
              </ul>
  ...
  <style>@import url('https://fonts.googleapis.com/css2?family=Montserrat:wght@400;600&display=swap');</style>
        ...
        console.log(`Письмо отправлено пользователю: ${user.email}`);
      } catch (err) {
        console.error(
          `Ошибка отправки письма для ${user.email}:`,
          err?.message || err,
        );
      }
    }
